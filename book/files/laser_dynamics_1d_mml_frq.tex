...
\delta \omega_q = -\frac{1}{2\, \tau_\lambda}\, \frac{\Im[f_q]}{\Re[f_q]}\, ,
 \end{equation}
...
% \begin{align}
% \label{eqn:mml_1d_cq_dot} \dot{c}_q(t) &= -\frac{1}{2\, \tau_\lambda}\, c_q(t) + \Re\left[f_q(t)\right] , \nd \\
...
In \sct{laser_statics_1d_approx}, we found that $\Im[f_0]/\Re[f_0] = \Im[\Lo]/\Re[\Lo]$, where $\Lo$ is the lineshape function, and we assume as a starting point that this condition is approximately valid in the multimode case. That is,
\begin{equation}
    \frac{\Im[f_q]}{\Re[f_q]} \approx \frac{\Im[\Lq]}{\Re[\Lq]} = \frac{\imlq}{\relq}\, ,
\end{equation}
where now $\relq = \Re[\Lq]$ and $\imlq = \Im[\Lq]$. Therefore, choosing
\begin{equation}
    \delta \omega_q \equiv -\frac{1}{2\, \tau_\lambda}\, \frac{\imlq}{\relq}
\end{equation}
should give $\dot{\phi}_q(t) \approx 0$ when $\dot{A}_q(t) = 0$.

In the general case, where
\begin{equation} \label{eqn:mml_1d_omega_general}
    \Lq \equiv \frac{\mathcal{A}}{\mathcal{B} - i\, \Omega}\, ,
\end{equation}

\begin{equation}
    \delta \omega_q  = -\frac{1}{2\, \tau_\lambda}\, \frac{\Im\left[\mathcal{A}\, \mathcal{B}^\ast\right] + \Re\left[\mathcal{A}\right] \left(\Omega_q + \delta \omega_q\, \tau_\perp\right)}{\Re\left[\mathcal{A}\, \mathcal{B}^\ast\right] - \Im\left[\mathcal{A}\right] \left(\Omega_q + \delta \omega_q\, \tau_\perp\right)}\, ,    
\end{equation}
where $\Omega_q = 2\, q\, \pi\, \tau_\perp$.

\begin{equation}
    \delta \omega_q \approx -\frac{1}{2\, \tau_\lambda + \tau_\perp} \left\{ \frac{\Im\left[\mathcal{A}\, \mathcal{B}^\ast\right]}{\Re\left[\mathcal{A}\, \mathcal{B}^\ast\right]} + \frac{\left|\mathcal{A}\right|^2\, \Re\left[\mathcal{B}\right] \Omega_q}{\Re\left[\mathcal{A}\, \mathcal{B}^\ast\right]^2} + \frac{\left|\mathcal{A}\right|^2\, \Im\left[\mathcal{A}\right]\, \Re\left[\mathcal{B}\right]\, \Omega_q^2}{\Re\left[\mathcal{A}\, \mathcal{B}^\ast\right]^2 \left(\Re\left[\mathcal{A}\, \mathcal{B}^\ast\right] - \Im\left[\mathcal{A}\right]\, \Omega_q\right)} \right\}
\end{equation}

If $\omega_0 = \omega_{a b}$, then
\begin{equation} \label{eqn:mml_1d_omega_q_def}
    \Omega_q \longrightarrow \Omega_q + \delta \omega_q\, \tau_\perp\, ,
\end{equation}
and we can solve for $\delta \omega_q$ to obtain
\begin{equation} \label{eqn:mml_1d_freq_pull_sol}
    \delta \omega_q \approx -\frac{r(\Omega_q)}{2\, \tau_\lambda + \tau_\perp\, r^\prime(\Omega_q)}\, ,
\end{equation}
where
\begin{subequations}
    \begin{align}
        r(\Omega_q) &\equiv \frac{\imlq}{\relq}\, , \nd \\
        r^\prime(\Omega_q) &\equiv \frac{d\, r(\Omega)}{d \Omega}\bigg|_{\Omega = \Omega_q}\, .
    \end{align}
\end{subequations}
Substituting \eqn{mml_1d_freq_pull_sol} into \eqn{mml_edot_temp} and simplifying, we find
\begin{equation} \label{eqn:mml_1d_deq_dt_fp}
    \begin{split}
        \dot{E}_q(t) &= -\frac{1}{2 \tau_\lambda} \left( 1 - i\, 2\, \delta \omega_q\, \tau_\lambda \right) E_q(t) + F_q(t) \\
        &= -\frac{1}{2 \tau_\lambda} \left[ 1 + i\, \frac{r(\Omega_q)}{1 + (\tau_\perp/2\, \tau_\lambda)\, r^\prime(\Omega_q)} \right] E_q(t) + F_q(t) .
    \end{split}
\end{equation}

In the Lorentzian case, where
\begin{equation} \label{eqn:mml_lmc_q_def}
    \Lq \equiv \frac{1}{1 - i\, \Omega_q}\, ,
\end{equation}
...
%In steady-state, then, $c_q(t) \approx 2\, \tau_\lambda\, \Re[f_q(t)]$, and \eqn{mml_1d_phiq_dot} becomes
% \begin{equation} \label{eqn:mml_1d_phiq_dot_approx}
%\dot{\phi}_q(t) \approx \delta \omega_q + \frac{\Omega_q}{2\, \tau_\lambda}\, .
% \end{equation}
we obtain
\begin{equation}\label{eqn:mml_1d_freq_pull}
    \delta \omega_q = -\frac{\tau_\perp}{2\, \tau_\lambda}\, \frac{2 q \pi}{1 + \tau_\perp/2\, \tau_\lambda}\, .
\end{equation}
...
\end{equation}
we find
\begin{equation}\label{eqn:mml_1d_freq_pull_general}
    \delta \omega_q = -\frac{\Re\left[\mathcal{A}\left(\mathcal{B}^\ast + i\, \Omega_q\right)\right]\, \Im\left[\mathcal{A}\left(\mathcal{B}^\ast + i\, \Omega_q\right)\right]}{2\, \tau_\lambda\, \Re\left[\mathcal{A}\left(\mathcal{B}^\ast + i\, \Omega_q\right)\right]^2 + \tau_\perp\, \left|\mathcal{A}\right|^2\, \Re\left[\mathcal{B}\right]}\, .
\end{equation}
...
\begin{equation} \label{eqn:mml_1d_disp_cont}
    i\, \sum_{l = 2}^\infty \frac{D_l\wn}{l!} \left(i\, \frac{\partial}{\partial t}\right)^l E\zt = i\, \sum_p u_p\z\, e^{-i\, \Delta \omega_p\, t} \left[ \delta D_p\wn\, E_p(t) + i\, \delta \tau_\lambda\wn\, \dot{E}_p(t) \right]\, ,
\end{equation}
where
\begin{subequations} \label{eqn:mml_1d_delta_dt_q_def}
    \begin{align}
        ...
        \delta \tau_\lambda\wn &\equiv \sum_{l = 2}^\infty \frac{(2 p \pi)^{l - 1}}{(l - 1)!}\, D_l\wn\, ,
    \end{align}
\end{subequations}
...
% \ddz u_q\z = i\, \left(\Delta \omega_q - \delta \omega_q\right) u_q\z + \left[\frac{1}{2\, \tau_\lambda} - \half\, \alpha\wn\right] u_q\z\, ,
%  \end{equation}
% we obtain
% % \begin{multline}
% %\sum_l u_l\z \left[ 1 + \delta \tau_l\wn \right] (-i \nu)\, E_l(\nu) = \\ \sum_l u_l\z \left\{-\frac{1}{2\, \tau_\lambda} + i \left[ \delta \omega_l + \sum_{m = 2}^\infty \frac{(2 l \pi)^m}{m!}\, D_m\wn \right] \right\} E_l(\nu) + F\zw\, ,
% % \end{multline}
%  \begin{multline} \label{eqn:mml_1d_nu_e}
% \sum_l u_l\z \left[ 1 + \delta \tau_l\wn \right] (-i \nu)\, E_l(\nu) = \\ \sum_l u_l\z \left\{-\frac{1}{2 \tau_\lambda} \left( 1 + i\, \Omega_l \right) + i\, \delta D_l\wn \right\} E_l(\nu) + F\zw\, ,
%  \end{multline}
% where we have chosen $\delta \omega_l = -\Omega_l/2\, \tau_\lambda$ to incorporate frequency pulling,
...
\begin{equation} \label{eqn:mml_1d_deq_dt_final}
    \dot{E}_q(t) = \frac{1}{1 + \delta \tau_q\wn} \left\{ \left[-\frac{1}{2\, \tau_\lambda} \left( 1 - i\, 2\, \delta \omega_q \tau_\lambda \right) + i\, \delta D_q\wn\right] E_q(t) + F_q(t) \right\}\, ,
\end{equation}
...
% \begin{equation}
%\left[ 1 + \delta \tau_q\wn \right] \dot{E}_q(t) = \\ \left\{-\frac{1}{2\, \tau_\lambda} + i \left[ \delta \omega_q + \sum_{m = 2}^\infty \frac{(2 q \pi)^m}{m!}\, D_m\wn \right] \right\} E_q(t) + F_q(t)\, ,
% \end{equation}
% \begin{equation} \label{eqn:mml_1d_deqdt}
%\left[ 1 + \delta \tau_q\wn \right] \dot{E}_q(t) = \\ \left\{-\frac{1}{2 \tau_\lambda} \left( 1 + i\, \Omega_q \right) + i\, \delta D_q\wn \right\} E_q(t) + F_q(t)\, ,
% \end{equation}

%Following the same procedure leading to \eqn{mml_1d_phiq_dot_approx}, we find
% \begin{equation}%\label{eqn}
%\delta \omega_q + \sum_{m = 2}^\infty \frac{D_m\wn}{m!}\, (2 q \pi)^m = -\frac{\Omega_q}{2\, \tau_\lambda}\, ,
% \end{equation}
...
% \begin{align}
%\label{eqn:mml_1d_freq_shift} \delta \omega_q &= -\frac{1}{1 + \tau_\perp/2\, \tau_\lambda} \left[\frac{\tau_\perp}{2\, \tau_\lambda}\, (2 q \pi) + \sum_{m = 2}^\infty \frac{(2 q \pi)^m}{m!}\, D_m\wn \right]\, , \nd \\
%\label{eqn:mml_1d_freq_disp} \Delta \omega_q &= \frac{1}{1 + \tau_\perp/2\, \tau_\lambda} \left[2 q \pi - \sum_{m = 2}^\infty \frac{(2 q \pi)^m}{m!}\, D_m\wn \right]\, .
% \end{align}
