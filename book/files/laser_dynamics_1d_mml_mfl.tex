...
\begin{equation} \label{eqn:mflt_kz_def}
    ...
    \end{split}
\end{equation}
Here we have followed the discussion in \sct{laser_statics_1d_approx} and defined $g_\mathrm{eff}(z)$ for the \emph{unwrapped} cavity as
\begin{equation}
...
\end{equation}
where from \eqn{sml_1d_u_url_nu_beta} $\beta$ in the standing-wave case is given by  
\begin{equation}
    \label{sml_1d_u_swl_nu_beta}
    \beta \equiv \frac{\ln(1/R_1\, R_2\, e^{-\overline{\alpha}_0})}{\overline{G}_0}\, ,
\end{equation}
and the unsaturated round-trip gain (through the unfolded cavity) and the total round-trip background loss are defined as
\begin{subequations}
    \begin{align}
        \Gnb &\equiv \int_0^1 d z\, \Gnz\, , \nd \\
        \anb &\equiv \int_0^1 d z\, \anz\, .
    \end{align}
\end{subequations}
We note that $\Gn(1 - z) = \Gnz$ because $\Gnz$ is symmetric about $z = 1/2$ in the unfolded cavity, and that $\Gn(1 + z) = \Gnz$ because the gain is also periodic, with the same observations applying to the background loss. In the general case over the coordinate domain $0 < z < 1$, the approximate expression for $K\z$ is given by
\begin{equation} \label{eqn:mflt_kz_genx}
    K\z \approx \left[ \theta(z)\, \theta(1/2 - z) + \theta(z - 1/2)\, \theta(1 - z)\, R_2 \right] \exp\left\{ \int_0^z d z^\prime \left[\beta\, \Gn\left(z^\prime\right) - \alpha_0\left(z^\prime\right) \right] \right\} \, ,
\end{equation}
and in the special case where $\Gnz = \Gnb$ and $\anz = \anb$ we have
...
Now we define the forward-propagating electric-field amplitude function $E(z,t)$ in terms of the single-mode steady-state intensity as
...
\begin{equation}
    \begin{split}
        \overline{K} &= \int_{0}^{1/2} d \zeta\, \exp\left\{\int_{0}^{\zeta} d z\, \left[\beta\, \Gnz - \anz\right] \right\} + R_2\, \int_{1/2}^{1} d \zeta\, \exp\left\{\int_{0}^{\zeta} d z\, \left[\beta\, \Gnz - \anz\right] \right\}\\
        &= \int_{0}^{1/2} d \zeta\, \exp\left\{\int_{0}^{\zeta} d z\, \left[\beta\, \Gnz - \anz\right] \right\} \\
        &\qquad + \sqrt{\frac{R_2}{R_1}}\, \int_{0}^{1/2} d \zeta\, \exp\left\{\int_{0}^{\zeta} d z\, \left[\beta\, G_0(z + 1/2) - \alpha_0(z + 1/2)\right] \right\} \, .
    \end{split}
\end{equation}

For the time being, let's assume that the unsaturated gain and the background loss are constants and fill the resonator, so that $\Gnz = \Gnb$ and $\anz = \anb$. In this case, we can use \eqn{mflt_kz_approx} to calculate the average value of the effective gain from \eqn{mflt_kbar_def} as
% \begin{equation}
%     \begin{split}
